...
std::vector<DetectBox> postprocess(const std::vector<std::vector<float>>& outputs, int image_h, int image_w) {
    ...
    for (int i = 0; i < anchors; i++) {
        ...
        if (cls_value > object_thresh) {
            ...
            float xmin = (meshgrid[gridIndex + 0] - cx) * strides[index];
            float ymin = (meshgrid[gridIndex + 1] - cy) * strides[index];
            float xmax = (meshgrid[gridIndex + 0] + cw) * strides[index];
            float ymax = (meshgrid[gridIndex + 1] + ch) * strides[index];
