...
std::vector<DetectBox> postprocess(const std::vector<std::vector<float>>& outputs, int image_h, int image_w) {
    ...
    for (int i = 0; i < anchors; i++) {
        ...
        if (cls_value > object_thresh) {
            ...
            else if (i < map_size[0][0] * map_size[0][1] + map_size[1][0] * map_size[1][1])
            {
                index = 1;
            }
            else
            ...
            //dfl过程
            regdfl.clear();
            int quant_zp_reg = out_zps[1];
            float quant_scale_reg = out_scales[1];
            for (int lc = 0; lc < 4; lc++)
            {
                ...
                {
                    locvaltemp = exp(output_reg[(lc * 16  + df) * anchors + i]);
            ...
            //特征图到原图的对齐
            float xmin = (meshgrid[gridIndex + 0] - regdfl[0]) * strides[index];
            float ymin = (meshgrid[gridIndex + 1] - regdfl[1]) * strides[index];
            float xmax = (meshgrid[gridIndex + 0] + regdfl[2]) * strides[index];
            float ymax = (meshgrid[gridIndex + 1] + regdfl[3]) * strides[index];
